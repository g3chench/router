...
The code was structured off the rough pseudocode described in the Programming Assignment 2 tutorial.

Our `sr_nat` struct contains a pointer to the sr instance used for the sake of convenience, passing fewer input parameters across functions that make a call to `handle_ICMP` which requires an instance of `sr`.
 
TCP Port assignment was done sequentially in a loop. A moving counter variable slides up to the next lowest numbered unassigned port.
