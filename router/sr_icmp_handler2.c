...
void gen_icmp_hdr (uint8_t* destination, int type, int code, uint8_t *old_pkt) {
	...
	sr_ip_hdr_t *ip_hdr packet_ip_hdr = (sr_ip_hdr_t *)(buf + sizeof(sr_ethernet_hdr_t));
	...
	if (type == 0) {
		...
		icmp_sum = 0;
		icmp_hdr->icmp_sum = cksum(icmp_hdr, ntohs(ip_hdr->ip_len) - (ip_hdr_t->ip_hl*4);
		...
		if ((code == 0) || (code == 1) || (code != 3)) {
			...
			fprintf("Unsupported ICMP code for type 3 ICMP.\n");
			return NULL;
		}
 ...
 */
void gen_ip_hdr(uint8_t* destination, uint32_t src, uint32_t dst, uint16_t cargo_size, uint8_t protocol) {
	...
	ip_hdr->ip_v = 4;
	ip_hdr->ip_tos = 0;
	ip_hdr->ip_len = htons(sizeof(sr_ip_hdr_t) + sizeof(cargo_size));
	ip_hdr->ip_id = 0;
 ...
 */
void gen_eth_hdr(uint8_t* destination, uint8_t src[ETHER_ADDR_LEN, uint8_t dest[ETHER_ADDR_LEN], uint16_t ether_type) {
	printf("\nIN FUNCTION: gen_eth_hdr--------------------------------------------\n");
	sr_ethernet_hdr_t *eth_hdr = (sr_ethernet_hdr_t *)(destination);
	memcpy(eth_hdr->ether_shost, source, ETHER_ADDR_LEN);
	memcpy(eth_hdr->ether_dhost, dest, ETHER_ADDR_LEN);
	eth_hdr->ether_type = htons(ether_type);

	memcpy(destination, sr_ethernet_hdr_t, sizeof(sr_ethernet_hdr_t));
}


void handle_icmp(int type, int code, struct sr_instance* sr, uint8_t* old_pkt, struct sr_if* iface, uint32_t sender) {
	printf("\nIN FUNCTION: gen_icmp_hdr--------------------------------------------\n");
	printf("Creating an ethernet frame containing an ICMP packet\n");


	struct sr_rt* rt_entry = lpm(original_ip_hdr->ip_src, sr->routing_table);
    if (!rt_entry) {
    	printf("Error: no LPM cannot find a matching arp entry\n");
    	return;
    }
	...
	int new_len;
	if (type == 3) {
		new_pkt_len = sizeof(sr_ethernet_hdr_t) + sizeof(sr_ip_hdr_t) + sizeof(sr_icmp_hdr_t);
		new_pkt = (uint8_t *)malloc(new_len);
	} else {
		new_pkt_len = sizeof(sr_ethernet_hdr_t) + sizeof(sr_ip_hdr_t) + sizeof(sr_icmp_t3_hdr_t);
		new_pkt = (uint8_t *)malloc(new_len);
	}
	printf("check that new_pkt has been initialized correctly: sizeof(new_pkt) = %", sizeof(new_pkt));
	...
	ip_hdr->ip_protocol = ip_protocol_icmp;
	...
	if (type == 0) {
		...
		gen_ip_hdr(ip_hdr, old_ip_hdr->ip_dest, old_ip_hdr->ip_src, sizeof(sr_icmp_hdr_t), ip_protocol_icmp);
		ip_hdr->ttl = INIT_TTL;
		ip_hdr->ip_sum = 0;
		ip_hdr->ip_sum = cksum(old_ip_hdr, old_ip_hdr->hl*4);

	} else if (type == 3 || type == 11) {
		printf("Create ip header for non 0 ICMP pkt...\n");
		gen_ip_hdr(ip_hdr, sender, old_ip_hdr->src, sizeof(sr_icmp_t3_hdr_t), ip_protocol_icmp)
	}
	print_hdr_ip(ip_hdr);

	/* CONSTRUCT ICMP HEADER */
	sr_icmp_hdr_t* icmp_hdr = new_pkt + sizeof(sr_ethernet_hdr_t) + sizeof(sr_ip_hdr_t);
	gen_icmp_hdr(icmp_hdr, type, code, old_pkt);
...
void send_icmp_echo_reply(uint8_t *packet, struct sr_if *interface) {
	handle_icmp(0, 0, packet, interface);
}

void send_icmp_echo_request(uint8_t *packet, struct sr_if *interface) {
	handle_icmp(8, 0, packet, interface);
}

void send_icmp_net_unreachable(uint8_t *packet, struct sr_if *interface) {
	handle_icmp(3, 0, packet, interface);
}
void send_icmp_host_unreachable(uint8_t *packet, struct sr_if *interface) {
	handle_icmp(3, 1, packet, interface);
}
void send_icmp_port_unreachable(uint8_t *packet, struct sr_if *interface) {
	handle_icmp(3, 3, packet, interface);
}

void send_icmp_time_exceeded(uint8_t *packet, struct sr_if *interface) {
	handle_icmp(11, 0, packet, interface);
}